...
const outputDir = process.env.SEO_CHECK_OUTPUT_DIR || path.join(root, '../docs/seo-2026-09-08');
...
(async()=>{
 ...
 fs.writeFileSync(path.join(outputDir,'correction-04-live-ssr.json'),JSON.stringify({checkedAt:new Date().toISOString(),note:'Local Server Component rendering with live public API. Not deployed HTTP route validation.',results},null,2));
