...
import { ArrowLeft, ArrowRight, ImageIcon, ShieldCheck, Wrench } from 'lucide-react'
...
export default async function ServiceDetailPage({ params, searchParams }: Props) {
  ...
  return <div className="bg-[#fbf8f0] dark:bg-background">
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      <Link href={`/${locale}/services`} className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-muted-foreground transition hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/40"><ArrowLeft className="size-4 rtl:rotate-180" aria-hidden="true" />{t('back')}</Link>

      <section className="mt-5 grid overflow-hidden rounded-[14px] border border-stone-200 bg-card shadow-[0_22px_60px_-46px_rgba(43,38,28,.75)] lg:grid-cols-[1.05fr_.95fr] dark:border-border">
        <div className="relative min-h-64 bg-muted sm:min-h-[25rem]">{image ? <Image src={image} alt={localized.name} fill priority sizes="(max-width:1023px) 100vw,55vw" className="object-cover" /> : <div className="absolute inset-0 flex items-center justify-center bg-amber-50 dark:bg-muted" role="img" aria-label={localized.name}><ImageIcon className="size-14 text-amber-700/45" /></div>}</div>
        <div className="flex flex-col p-6 sm:p-9 lg:p-10"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.18em] text-amber-800 dark:text-amber-300"><Wrench className="size-4" aria-hidden="true" />{t('eyebrow')}</p><h1 className="mt-4 text-4xl font-black tracking-[-.035em] sm:text-5xl">{localized.name}</h1><div className="mt-4 flex flex-wrap gap-2" aria-label={t('categories')}>{service.categories.map((category) => <span key={category.id} className="rounded-[4px] bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">{localizedCategoryName(category, locale)}</span>)}</div><p className="mt-5 whitespace-pre-wrap text-base leading-7 text-muted-foreground">{localized.description}</p><div className="mt-auto grid gap-3 pt-7 sm:grid-cols-2"><Button asChild size="lg" className="min-h-12 rounded-[6px]"><Link href={serviceRequestHref(locale, id)}>{t('requestService')}<ArrowRight className="size-4 rtl:rotate-180" /></Link></Button><Button asChild size="lg" variant="outline" className="min-h-12 rounded-[6px]"><Link href={quickServiceRequestHref(locale)}>{t('quickRequest')}</Link></Button></div></div>
      </section>

      <aside className="mt-6 flex items-start gap-3 border-s-2 border-emerald-700 px-4 py-2 text-foreground"><ShieldCheck className="mt-0.5 size-5 shrink-0 text-emerald-700" aria-hidden="true" /><div><h2 className="text-sm font-bold">{t('assuranceTitle')}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{t('assuranceText')}</p></div></aside>
      ...
      <section className="mt-12" aria-labelledby="compatible-maalems"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-foreground/10 pb-5"><div><h2 id="compatible-maalems" className="text-3xl font-black tracking-tight">{t('maalemsTitle')}</h2>{pagination && <p className="mt-1 text-sm text-muted-foreground">{t('maalemsCount', { count: pagination.total_items })}</p>}<p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{t('availabilityNotice')}</p></div></div>
        {!compatible ? <div className="mt-6 border border-destructive/20 bg-card p-8 text-center"><h3 className="font-bold">{t('errorTitle')}</h3><Button asChild variant="outline" className="mt-4"><Link href={href(page)}>{t('page', { page })}</Link></Button></div> : compatible.maalems.length === 0 ? <div className="mt-6 border border-dashed border-amber-300 bg-card p-8 text-center"><h3 className="text-xl font-bold">{t('emptyTitle')}</h3><p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">{t('emptyText')}</p><Button asChild className="mt-5"><Link href={serviceRequestHref(locale, id)}>{t('requestService')}</Link></Button></div> : <div className="mt-6 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{compatible.maalems.map((maalem) => <CompatibleMaalemCard key={maalem.id} maalem={maalem} locale={locale} serviceId={id} labels={{ verified: t('verified'), city: t('city'), areas: t('areas'), missions: t('missions', { count: maalem.closed_interventions_for_service }), profile: t('profile'), choose: t('choose'), rating: t('ratingLabel', { rating: '{rating}', count: '{count}' }), verifiedReviews: t('verifiedReviews'), noReviews: t('noReviews') }} />)}</div>}
