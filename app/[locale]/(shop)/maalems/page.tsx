...
export default async function MaalemsPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<Query> }) {
  ...
  return <main className="min-h-[70vh] bg-[#fbf8ef] py-10 dark:bg-background sm:py-14">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <section className="rounded-b-[14px] border border-stone-200 bg-card p-4 shadow-[0_16px_40px_-34px_rgba(75,55,20,.45)] sm:p-5 dark:border-border" aria-label={t('search')}>
        <form action={`/${locale}/maalems`}>
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-[minmax(0,1.25fr)_minmax(180px,.75fr)_minmax(180px,.75fr)_auto] lg:items-end">
            <label className="grid gap-1.5 text-sm font-semibold">{t('search')}<span className="relative"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><input suppressHydrationWarning name="q" defaultValue={filters.q} className="h-11 w-full rounded-[6px] border border-input bg-background px-9 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20" /></span></label>
            <label className="grid gap-1.5 text-sm font-semibold">{t('category')}<select name="category_id" defaultValue={filters.category_id || ''} className="h-11 rounded-[6px] border border-input bg-background px-3 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20"><option value="">{t('allCategories')}</option>{data?.filters.categories.map((item) => <option key={item.id} value={item.id}>{localizedCategoryName(item, locale)}</option>)}</select></label>
            <label className="grid gap-1.5 text-sm font-semibold">{t('service')}<select name="service_id" defaultValue={filters.service_id || ''} className="h-11 rounded-[6px] border border-input bg-background px-3 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20"><option value="">{t('allServices')}</option>{data?.filters.services.map((item) => <option key={item.id} value={item.id}>{localizedCategoryName(item, locale)}</option>)}</select></label>
            <div className="flex gap-2"><Button type="submit" className="min-h-11 rounded-[6px]"><Search className="size-4" />{t('submit')}</Button>{hasFilters && <Button asChild variant="outline" className="min-h-11 rounded-[6px]"><Link href={`/${locale}/maalems`}>{t('reset')}</Link></Button>}</div>
          </div>

          <details className="group mt-4 border-t border-stone-200 pt-4 dark:border-border" open={hasAdvancedFilters}>
            <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 text-sm font-semibold text-foreground outline-none focus-visible:ring-2 focus-visible:ring-amber-500/30 [&::-webkit-details-marker]:hidden">
              <SlidersHorizontal className="size-4 text-amber-700" aria-hidden="true" />{t('advancedFilters')}
            ...
            <div className="grid gap-3 pt-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              <label className="grid gap-1.5 text-sm font-semibold">{t('city')}<input suppressHydrationWarning name="city" defaultValue={filters.city} className="h-11 rounded-[6px] border border-input bg-background px-3" /></label>
              <label className="grid gap-1.5 text-sm font-semibold">{t('zone')}<input suppressHydrationWarning name="zone" defaultValue={filters.zone} className="h-11 rounded-[6px] border border-input bg-background px-3" /></label>
              <label className="grid gap-1.5 text-sm font-semibold">{t('experience')}<input suppressHydrationWarning type="number" min="0" max="70" name="min_experience" defaultValue={filters.min_experience} className="h-11 rounded-[6px] border border-input bg-background px-3" /></label>
              <label className="grid gap-1.5 text-sm font-semibold">{t('interventions')}<input suppressHydrationWarning type="number" min="0" name="min_interventions" defaultValue={filters.min_interventions} className="h-11 rounded-[6px] border border-input bg-background px-3" /></label>
              <label className="grid gap-1.5 text-sm font-semibold">{t('minimumRating')}<select name="min_rating" defaultValue={filters.min_rating || ''} className="h-11 rounded-[6px] border border-input bg-background px-3"><option value="">{t('allRatings')}</option>{[5,4,3,2,1].map((rating) => <option key={rating} value={rating}>{t('minimumRatingOption', { rating })}</option>)}</select></label>
      ...
      {!data ? (
        <section className="mt-8 rounded-[14px] border border-stone-200 bg-card p-5 shadow-[0_14px_34px_-30px_rgba(41,37,32,.5)] sm:flex sm:items-center sm:gap-5 sm:p-6 dark:border-border" role="alert">
          ...
          <div className="mt-4 min-w-0 flex-1 sm:mt-0"><p className="font-bold">{t('error')}</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{t('quickDescription')}</p></div>
        ...
        <section className="mt-10 rounded-[14px] border border-dashed border-amber-300 bg-card p-8 text-center sm:p-10"><p className="font-bold">{t('empty')}</p>{hasFilters && <Button asChild variant="outline" className="mt-5 min-h-11 rounded-[6px]"><Link href={`/${locale}/maalems`}>{t('reset')}</Link></Button>}</section>
      ) : (
        <section className="mt-9"><p className="mb-5 text-sm leading-6 text-muted-foreground">{t('declared')}</p><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{data.maalems.map((maalem) => <MaalemDirectoryCard key={maalem.id} maalem={maalem} locale={locale} stats={maalem.statistics} labels={labels} />)}</div></section>
      )}

      {data && <aside className="mt-10 flex flex-col gap-4 rounded-[14px] border border-emerald-800/20 bg-emerald-950 px-6 py-7 text-white sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-bold">{t('quick')}</h2><p className="mt-1 text-sm text-white/70">{t('quickDescription')}</p></div><Button asChild className="min-h-11 rounded-[6px] bg-amber-400 text-emerald-950 hover:bg-amber-300"><Link href={quickServiceRequestHref(locale)}>{t('quick')}<ArrowRight className="size-4 rtl:rotate-180" /></Link></Button></aside>}
