      ...
      <div className="flex flex-col gap-4 max-h-[400px] overflow-y-auto pr-2">
        {items.map((item) => (
          <div key={`${item.productId}-${item.variantId || ""}`} className="flex gap-3">
            <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-muted flex-shrink-0 border border-border/30">
              {item.image ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 mb-1">
                ...
                {showConfirmButton && (
                  <span className="px-2 py-0.5 rounded-full bg-amber-500 text-white text-xs font-bold flex-shrink-0">
                    x{item.quantity}
                  </span>
                )}
              </div>
      ...
      {showConfirmButton && onConfirmOrder && (
        <div className="mt-6 pt-4 border-t border-border/50">
          <Button
            ...
            disabled={isPending}
            className="w-full h-11 bg-gradient-to-r from-primary via-primary to-primary/90 hover:from-primary/90 hover:via-primary hover:to-primary text-white shadow-lg shadow-primary/25 font-semibold"
          >
            {isPending ? (
              <>
